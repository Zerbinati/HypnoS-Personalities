...
class ChessPersonalityAnalyzer(QWidget):
    ...
    def determine_playstyle(self, analysis_results):
        ...
        # 🔥 **Reduce Universal dominance if another style is close**
        if best_fit == "universal" and second_best_score > best_score * 0.9:
            print(f"\n⚠️ Universal dominance reduced, selecting {second_best_fit.upper()} instead")
    ...
    def suggest_json_update(self, analysis_results):
        ...
        if reply == QMessageBox.Yes:
            ...
            for key, deviation in analysis_results.items():
                ...
                # 🔥 Stronger correction for large deviations
                correction = round((deviation / 250) ** 1.15 / correction_factor)
